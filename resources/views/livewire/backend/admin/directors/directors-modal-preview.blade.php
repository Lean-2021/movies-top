<!-- Main modal -->
<div id="directors{{$row}}-card" tabindex="-1" aria-hidden="true"
    ...
    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700 drop-shadow-lg">
      ...
      <div class="flex items-center justify-between p-2 md:p-2 border-b rounded-t dark:border-gray-600">
        <button type="button"
                ...
                data-modal-hide="directors{{$row}}-card">
      ...
      <div class="p-2 md:p-5 space-y-4">
        <div
          ...
          <div class="flex flex-col items-center py-12">
            <img class="w-24 h-24 mb-3 rounded-full shadow-lg" src="{{asset('images/avatar.png')}}"
                 alt="Bonnie image"/>
            <h5
              class="mb-1 text-xl font-medium text-gray-900 dark:text-white">{{\Illuminate\Support\Str::title($director->first_name)}} {{\Illuminate\Support\Str::title($director->last_name)}}</h5>
            <p class="flex my-5 text-lg items-center"><img
                src="{{asset( 'storage/flags/'.$director->country->flag)}}"
                class="w-12 me-2"
                alt="{{$director->country->name}}">
              {{ \Illuminate\Support\Str::title($director->country->name)}}
            </p>
            <span class="text-xl text-gray-500 dark:text-gray-400">Director</span>
          </div>
        </div>
      </div>
    </div>
