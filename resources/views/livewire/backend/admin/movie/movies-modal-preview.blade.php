            ...
            @if ($movie)
                <div class="p-2 space-y-4 md:p-5">
                    <div
                        ...
                        <div class="grid grid-cols-1 px-5 py-12 sm:grid-cols-2 sm:gap-x-8">
                            <div>
                                {{-- imágen --}}
                                <div class="mb-3">
                                    <img class="object-cover h-64 rounded-lg shadow-lg w-80" {{--                     {{dd($movie)}} --}}
                                        src="{{ $movie->image && Storage::exists('movies/' . $movie->image) ? asset('storage/movies/' . $movie->image) : asset('images/no-image.jpg') }}"
                                        alt="{{ $movie->title }}" />
                                </div>
                                {{-- descripción - no visible en los dispositivos móbiles --}}
                                <div class="hidden mb-3 sm:block">
                            ...
                            <div>
                                {{-- título --}}
                                <h5 class="mt-4 mb-2 text-xl font-medium text-gray-900 sm:mt-0 dark:text-white">
                                ...
                                {{-- descripción - visible en dispositivos móbiles --}}
                                <div class="block mb-3 sm:hidden">
                                    <div class="group/description">
                                        <p>Descripción:</p>
                                        <p
                                            class="mt-2 text-justify text-gray-400 transition-all duration-300 text-wrap line-clamp-3 group-hover/description:line-clamp-none">
