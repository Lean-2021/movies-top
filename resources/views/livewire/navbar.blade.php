    ...
    <nav class="header_nav_links">
        ...
        @if (!request()->routeIs('register') && !request()->routeIs('login'))
            <div class="flex items-center gap-x-3 btn_menu_toggle">
                ...
                {{-- si el usuario esta logeado botón con imágen para dispositivos móbiles - administración usuario --}}
                @if (Auth::check())
                    <button id="dropdownmobile" data-dropdown-toggle="dropdownMobile"
                        class="flex text-sm bg-gray-800 rounded-full md:hidden md:me-0" type="button">
                        <span class="sr-only">Open user menu</span>
                        {{-- <img class="w-8 h-8 rounded-full" src="/docs/images/people/profile-picture-3.jpg" alt="user photo"> --}}
                        <img src="{{ Auth::user()->profile_photo_path && asset('storage/' . Auth::user()->profile_photo_path) ? asset('storage/' . Auth::user()->profile_photo_path) : asset('img/user_profile.png') }}"
                            alt="{{ Auth::user()->name }}"
                            class="z-10 w-9 h-9 object-cover border-2 rounded-md drop-shadow-[0px_2px_3px_rgba(0,0,0,0.7)] hover:grayscale">
                    </button>
                ...
                @if (auth()->check())
                    <li class="flex items-center header_items gap-x-2">
                        <span><img
                                ...
                                alt="{{ Auth::user()->name }}"
                                class="w-10 h-10 object-cover hidden md:inline-flex border-2 rounded-md drop-shadow-[0px_2px_3px_rgba(0,0,0,0.7)] hover:grayscale"></span>
